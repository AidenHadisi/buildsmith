...
- [ ] **Discoverable** — proof: `bunx buildsmith --help` lists every command group; `bunx buildsmith task --help` and `bunx buildsmith doc write --help` document args and flags; unknown command or missing required arg → usage on stderr, exit 1.
- [ ] **Board read path** — proof: `bunx buildsmith task list | jq` returns the two seed tasks with `column` and `next.stage`; `task get <A>` and `next <A>` agree with `GET /api/tasks/<A>`; the same commands in a TTY print text, not JSON.
...
- [ ] **Short ids** — proof: `task get e1b5d5` resolves task A; `task get 01a09815` (matches both seeds) → stderr "ambiguous", exit 1; `task get nope` → stderr not found, exit 1.
- [ ] **Store errors surface cleanly** — proof: `doc status <A> spec draft` → stderr contains `cannot move spec status`, exit 1; with `--json`, stderr is `{"error":"..."}`; no stack trace in either.
- [ ] **Body input** — proof: `doc write <id> spec --file f.md` and `cat f.md | buildsmith doc write <id> spec` yield byte-identical `spec.md` bodies; with a TTY stdin and no `--file` → usage error, exit 1.
- [ ] **Green checks** — proof: `bun run check`, `bun run typecheck`, `bun test` (incl. CLI spawn tests), root `bun run build` pass; `ls node_modules/.bin/buildsmith` exists after `bun install`.
...
- [ ] `doc write <id> spec --file f.md` and `cat f.md | doc write <id> spec` produce byte-identical `spec.md`; `stdin: "ignore"` and no `--file` → `empty body…`, exit 1; `doc read <id> spec` before any write → `null` JSON / nothing in text, exit 0.
- [ ] `slice add <id> --title T --goal G "c1" "c2"` → `slice list` shows `criteria: ["c1","c2"]`; `slice update`, `note add/list`, `project read/write/lesson`, `asset put` each round-trip through the store; `doc status <A> spec draft` → `cannot move spec status…` on stderr, exit 1.
- [ ] Full pipeline (criterion 3) in a temp repo drives `next` to `done`; `BUILDSMITH_ROOT=<tmp>` web server serves `/api/tasks/<id>`.
- [ ] `bun test` covers the pipeline end to end; all checks green.
