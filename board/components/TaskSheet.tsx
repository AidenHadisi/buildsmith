        ...
        <SheetHeader>
          ...
          {data && (
            <div className="flex items-center gap-3 text-xs text-muted-foreground">
              <span className="font-mono">{data.task.id}</span>
...
function SheetBody({ data }: { data: TaskDetail }) {
  ...
  const building = next.stage === "building";
  const polishing = next.stage === "polish" || next.stage === "review";
  return (
    <Tabs defaultValue="overview" className="px-4 pb-4">
      <TabsList className="w-full flex-wrap group-data-horizontal/tabs:h-auto">
        ...
        <TabsTrigger value="spec">
          Spec
          <StepMark done={spec?.status === "approved"} current={next.stage === "spec"} />
        </TabsTrigger>
        <TabsTrigger value="architecture">
          Architecture
          <StepMark
            done={architecture?.status === "approved"}
        ...
        <TabsTrigger value="slices">
          Slices
          <StepMark
            done={slices.length > 0 && sliceDone === slices.length && !polishing}
            current={building || polishing}
        ...
        <TabsTrigger value="verification">
          Verification
          <StepMark
            fail={verification?.result === "fail"}
            done={verification?.result === "pass"}
...
function Overview({ data: { task, next } }: { data: TaskDetail }) {
  return (
    <div className="space-y-4">
      <Section title="Next">
        <p className={next.ask ? "text-sm text-warning" : "text-sm"}>
          <span className="font-medium">{label(next.action)}</span> — {next.ask ?? next.reason}
        </p>
      </Section>
      {next.blocked?.length ? (
