...
export function wasSelfWrite(path: string, text: string): boolean {
  ...
  const hit = selfWrites.get(key) === Bun.hash(text);
  if (hit) selfWrites.delete(key);
  return hit;
}
...
export function stringifyRecord(data: Record<string, unknown> | Document, body: string): string {
  const fm = (data instanceof Document ? data : new Document(data)).toString({ lineWidth: 0 });
  const yamlOut = fm.endsWith("\n") ? fm : `${fm}\n`;
  return `---\n${yamlOut}---${body.startsWith("\n") ? body : `\n${body}`}`;
}

function parseRecord<T>(path: string, schema: z.ZodType<T>, raw: string) {
  const split = splitFrontmatter(raw);
  if (split.fm === null) throw new Error(`no frontmatter in ${path}`);
  const doc = parseDocument(split.fm);
  ...
  return { data: parsed.data, doc, body: split.body };
}
...
/** Git-style lockfile: write to `path.lock` with O_EXCL, then rename over `path`. Returns the written text. */
export async function write(
  ...
  const text = await waitFor(lock, async () => {
    await using fh = await open(lock, "wx");
    try {
...
export function record<T>(path: string, schema: z.ZodType<T>) {
  const parse = (raw: string) => parseRecord(path, schema, raw);
  const read = async () => {
  ...
  return {
    ...
    async patch(
      fields: Record<string, unknown> | ((data: T) => Record<string, unknown>),
      body?: string,
    ) {
      const out = await write(path, (raw) => {
        if (raw === null) throw new Error(`missing file ${path}`);
        const rec = parse(raw);
        const next = typeof fields === "function" ? fields(rec.data) : fields;
        for (const [key, value] of Object.entries(next)) {
          if (value !== undefined) rec.doc.set(key, value);
        }
        return stringifyRecord(rec.doc, body ?? rec.body);
      });
      return parse(out);
    },
